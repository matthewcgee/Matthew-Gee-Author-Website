...
export default function StatusBoard({ locations, entries, thresholds }) {
  const summaries = locations.map((loc) => {
    ...
    return { loc, latest, value, stage, trend }
  })
  ...
  return (
    <div>
      ...
      <div style={grid(3)}>
        {summaries.map(({ loc, latest, value, stage, trend }, idx) => {
              ...
              {latest ? (
                <div style={{ fontSize: 12, color: theme.sub }}>
                  ...
                  {latest.addStaffNext != null && latest.addStaffNext !== '' && (
                    <div style={{ marginTop: 4, color: theme.accent, fontWeight: 600 }}>
                      Recommend adding {latest.addStaffNext} staff next shift
                    </div>
                  )}
                </div>
              ) : (
