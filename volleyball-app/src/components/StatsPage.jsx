...
function buildRow(state, player) {
  const { totals, gamesPlayed } = computeSeasonTotals(state.events, player.id)
...
function exportCsv(rows) {
  ...
  a.href = url
  a.download = `efhs-volleyball-season-stats-${new Date().toISOString().slice(0, 10)}.csv`
    ...
    <>
      ...
      <Card eyebrow="Legend" title="What the columns mean">
        <p style={{ margin: 0, color: 'var(--silver)', fontSize: 12.5, lineHeight: 1.7 }}>
          <strong>GP</strong> games with a logged stat &middot; <strong>K</strong> kills &middot;{' '}
