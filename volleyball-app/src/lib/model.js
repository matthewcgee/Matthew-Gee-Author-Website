...
// Replay a game's event log into the live/derived state a screen needs:
...
export function deriveGameState(events) {
  let us = 0
  let them = 0
  let setNumber = 1
  const completedSets = []
...
export function computeSeasonTotals(events, playerId) {
  ...
  for (const e of events) {
    if (e.type === 'stat' && e.playerId === playerId) {
      totals[e.statKey] += 1
      gamesPlayed.add(e.gameId)
    }
  }
  return { totals, gamesPlayed: gamesPlayed.size }
}
